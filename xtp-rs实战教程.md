...
# QUIC 探针（RTT/丢包/MTU）在总分中的权重，默认 40：TCP 吞吐 60% + QUIC 探针 40%
# 想更倚重真实吞吐表现就调低（如 20），想更敏感于链路质量就调高
quic_weight = 40
...
- 两路信息都有效时：`总分 = TCP分 × (100-quic_weight)% + QUIC分 × quic_weight%`；只有一路有效就用那一路；都没有时按**基础分 75** 处理。
- 每条新连接做 **平方加权随机**：权重 = 分²。例如 BBR 800 分、Brutal 400 分，选中概率 800²:400² = **80% : 20%**——强者通吃大部分新连接，弱者仍保有少量「探测流量」以便网络恢复时能重新涨分。
- 连接失败的上游会被**惩罚**到基础分 75，基本出局，直到健康检查或新流量把它捞回来。
...
| `quic_weight` | `40` | QUIC 探针权重；大流量场景调低更倚重实测吞吐 |
| `quic_stale_secs` | `180` | QUIC 探针分数过期阈值（秒），超期后线性衰减到最低分 10 |
| `enable_relative_scoring` | `false` | 跨链路相对归一化评分，见 6.7 节 |
| `relative_rescore_interval_secs` | `2` | 相对模式的重算间隔（秒） |
...
### 6.7 相对归一化模式：光纤 / 4G / 卫星同台竞技

绝对分段评分有个隐含前提：**所有上游处于同一量级的网络**。吞吐阶梯（≥50 MiB/s 才满分）是为光纤设计的——一条 500 MiB 光纤、一条 50 MiB 4G 时，4G 无论多稳定都只能拿零头分，永远没有出头之日。

`enable_relative_scoring = true` 开启相对模式，评分口径从「绝对阈值」换成「**相对同组最大值**」：

```toml
enable_relative_scoring = true
relative_rescore_interval_secs = 2   # 重算间隔，默认 2 秒
```

- 每隔 `relative_rescore_interval_secs`，把每条上游的实测吞吐速率、探针质量各自**除以组内最大值**映射到 0~1000：组内最快那条得满分，其余按比例得分。评分口径与绝对量纲无关——1 MiB/s 的 4G 和 500 MiB/s 的光纤同台竞争时，谁在**自己网络里跑得最满**谁得分高。
- **冷启动种子**：还没有吞吐样本的新链路，用探针相对分预置初始分，立即具备竞争力；连探针报告都拿不到（新链路没分到流量，ShadowQUIC 只在有流量时上报）时，用组内最优的一半做乐观先验——先按「可能不错」对待，拿到真实样本后分数自然修正。
- **未初始化探索**：存在无任何测量样本的上游时，选路有 5% 的概率从这些上游里均匀选一条，保证它一定能分到流量、产生样本。探针和吞吐都靠流量喂养，不探索的话新上游永远拿不到分——这正是相对模式初版踩过的坑：新链路卡在基础分 75，被高分链路平方加权碾压到 1% 选中率，陷入「无样本 → 无分数 → 无流量」的饥饿死锁。两条机制配合，死锁彻底打破。
- 坏链路成本有上限：新上游第一次连接失败就触发惩罚，打回基础分 75 并标记为已测量，探索和种子立即停止向它倾斜。

> [!TIP]
> 默认 `false`，不影响现有部署，可随时平滑回退。**多网络类型混合、或新加上游希望立即参与竞争的场景建议开启**。
...
# ═══════════ 动态竞争（第 6 章）═══════════
disable_upstream_score = false
# enable_relative_scoring = true       # 相对归一化评分（6.7 节），多网络混搭或新上游即时参与竞争时开启
# quic_weight = 40                     # 默认 40；想更倚重实测吞吐就调低
...
默认 40；大流量下载 / 视频场景可以再调低，让实测吞吐主导；玩实时游戏在意 RTT 可以拉高，更偏链路质量。
